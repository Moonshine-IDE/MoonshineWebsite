...
This is going to install Apache Royale to the Moonshine's default SDK folder which is:
- `C:\MoonshineSDKs` on Windows
- ... on Mac

#### Creating Simple Jewels App
Jewels is a UI component set for Apache Royale. We're going to levrage this beautiful framewkork to create a simple yet elegant looking form.
...
- From the dropdnown select `Royale Browser Project Jewel`
- Make sure Royale 0.9.8 nightly is set as your SDK
...
###### Step 6
In the Project View on the left locate `src/MainContent.mxml` file.
...
###### Step 9
This should compile the project and start a built in server. It can alos open up a browser window with your project.
...
###### Step 10
Go to `src/view/Content.mxml` file. Remove all the unneccesary tags inside VGroup and instead creata a simple card section:
...
###### Step 11
Before we move forward we need to add html namespace attribute to VGroup tag:
...
###### Step 13
Next, we modify our CardPrimaryContent to take a Form with three FormItems -- one for first name, one for last name and one for email.
...
###### Step 16
In order to use JavaScript code you need to create a pair of Script tags (from already included fx namespace). Those tags shuld be placed inside your root VGroup tag.

Then, the script itself needs to be put inside CDATA syntax.
...
###### Step 17
Let's show a simple alert when the button is clicked. Add the following imports and method inside CDATA:
...
###### Step 20
Now let's try to build some JSON out ouf the data in our form and displaying it in the alert.
...
Let's add a beads section inside VGroup, and then an HTTP service pointing to our Grails Back End we're going to build in the next part.
...
The url is a location of our Back End. For the purpose of this tutorial we will be building Grails Back End at exactly this address.

###### Step 22
Using HTTPService requires adding also js namespace:
...
###### Step 23
As you may noticed we used `complete` and `ioError` events on HTTPService. Let's implement their handlers.
...
As you can see, the last response is stored in the `service.data` object and you can access it in `onComplete` handler.
...
You CAN actually run two server instances with Moonshine at the same time:
- Grails runs by default at: http://localhost:8080/
- Royale runs by dafault at: http://localhost:3000/

###### Step 25
The response we are receiving is not very readable in it's current state. It may help you with tesitng if we made it a little prettier. Go to the `onComplete` callback. We're going to:
- Indent the JSON for better readability. Because the response is already in JSON format the easiest way to do this is to convert it to object and back to JSON with approprieate options.
...
- pass the token togeter with the data to the Back End
...
Substitute SITEK_KEY with the one you obtained durign reCAPTCHA registration. This is a public key, so it will be visible to all the people browsing your page.

![](img/site-key.png)

###### Step 27
Build the project and refresh your page. You should see "protected by recaptcha" logo.
...
###### Step 29
 Create new method `sendRequest`. Make it take a token of type string and return void. Now cut everything from `onSignupClick` and paste it to the `sendRequest`. Adjust your JSON to contain both user and the tokne. This should is how the final version of this method looks like:
...
###### Step 30
Now go back to `onSignupClick` method. We need it to:
- find google recaptcha object embeded in your site by the script we included earlier
- make it invoke `execute` method on that object, passing your site key, and optionally the name of the action your trying to protect
- receive the token as response
- pass it to `sendRequest` method.

This seems like a lot but in reality it's just almos a one-line implementation:
...
#### More Testing
You if hava a working Back End in Grails you can now test the two applications together. This is how a successful response looks like:
